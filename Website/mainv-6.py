...
########################################
interactive_tasks = {
    # Replace the task "3" entry in the interactive_tasks dictionary with:
"3": {
    "title": "Task 3 Interactive: Reflection Travel Time",
    "sliders": [
        {"id": "v", "label": "Speed: ", "min": 1, "max": 8.48, "value": 8.48, "step": 0.01},
        {"id": "n", "label": "Refractive Index: ", "min": 1, "max": 3, "value": 1, "step": 0.1},
        {"id": "y", "label": "Height: ", "min": 1, "max": 10, "value": 1, "step": 1},
        {"id": "l", "label": "Length: ", "min": 0.1, "max": 3.0, "value": 1.0, "step": 0.01}
    ],
    "plot_endpoint": "/plot/3",
    "banned_validation": False
},
    # Replace the task "4" entry in the interactive_tasks dictionary with:
"4": {
    ...
    # Replace the task "5" entry in the interactive_tasks dictionary with:
"5": {
    "title": "Task 5 Interactive: Virtual Image Plot",
    "sliders": [
        {"id": "offset_x", "label": "Start X: ", "min": -max(W, H), "max": max(W, H), "value": int(img_width / 2) + 1, "step": 1},
        {"id": "offset_y", "label": "Start Y: ", "min": -max(W, H), "max": max(W, H), "value": 0, "step": 1},
    ...
    # Replace the task "6" entry in the interactive_tasks dictionary with:
"6": {
...
def generate_task4_plot(v_log, n1, n2, y, l, request_id):
    try:
        ...
        # Create a LaTeX-formatted title with equals signs
        title_text = (r"$\sin(\theta_1)/(v/n_1) = " + f"{round(np.sin(theta1)/(v/n1),15)}" +
                      r",\quad \sin(\theta_2)/(v/n_2) = " + f"{round(np.sin(theta2)/(v/n2),15)}" + r"$")
        plt.figure()
        plt.scatter(x[idx], t[idx], color="red", zorder=2)
        plt.plot(x, t, zorder=1)
    ...
    try:
        ...
        for y in range(H):
            ...
            for x in range(W):
                ...
                # Compute positions for two copies of the image
...
def generate_task10_plot(Rf, arc_angle, request_id):
    try:
        ...
        # Use center = (0, y_center) with y_center = -img_height/2
        ...
        # For each row in the image, generate an arc with interpolated colors.
        ...
        for row_index in range(img_height):
            ...
            R_here = R_f_slider * ((img_height - row_index - 1) / img_height) + 1
            theta = np.linspace(start_angle, end_angle, 300)
            x_center = 0
            y_center = -img_height / 2  # New center: negative half the image height
            ...
            # Interpolate color from the corresponding row of the image
            row_data = global_image[row_index]
            if row_data.max() > 1:
                row_data = row_data / 255.0
            interp_indices = np.linspace(0, img_width - 1, len(theta))
            segment_colors = []
            for i in range(len(theta) - 1):
                pos = interp_indices[i]
                color = []
                for ch in range(row_data.shape[1]):
                    c = np.interp(pos, np.arange(img_width), row_data[:, ch])
                    color.append(c)
                segment_colors.append(color + [1])
            lc = LineCollection(segments, colors=segment_colors, linewidth=3)
