...
def generate_task12a_plot(ThetaI_deg_slider, alpha_deg_slider, canvas_scale_12a, request_id): # Added canvas_scale_12a
    ...
    try:
        ...
        base_xlim = (-1.6, 1.6)
        base_ylim = (-0.8, 1.2)
        x_min, x_max = base_xlim
        y_min, y_max = base_ylim
        x_min *= canvas_scale_12a
        x_max *= canvas_scale_12a
        y_min *= canvas_scale_12a
        y_max *= canvas_scale_12a
        ThetaI_deg_slider = (ThetaI_deg_slider + 180 - alpha_deg_slider / 2 - 180) % 360
        check_interrupt("12a", request_id)
        
        alpha_rad = np.deg2rad(alpha_deg_slider)     
        ThetaI_abs_rad = np.deg2rad(ThetaI_deg_slider)

        # Banned validation: ThetaI_deg_slider <= 90 and ThetaI_deg_slider >= 90-alpha/2
        ...
        for i in range(len(frequencies)):
            ...
            if abs(sin_r1_signed) > 1.0: 
                ...
                all_segments_incident.append([[P1_x, P1_y], [P0_x, P0_y]])
        ...
        # Assume x_min, x_max, y_min, y_max are defined (canvas boundaries)
        # Assume ax is your matplotlib Axes object
        # Assume np and LineCollection are imported, and extend_to_edge is defined as above.

        # --- Incident Segments ---
        if all_segments_incident:
            extended_segments_incident = []
            for segment in all_segments_incident:
                p_start, p_direction = segment  # e.g., P0 (source) and P1 (hits object/defines direction)
                
                # Extend from p_start in the direction of p_direction to the canvas edge
                extended_p_end = extend_to_edge(p_start, p_direction, x_min, x_max, y_min, y_max)
                
                if extended_p_end:
                    extended_segments_incident.append([p_start, extended_p_end])
                else:
                    # Optional: handle cases where extension is not possible or segment is not drawn
                    # print(f"Could not extend incident segment: {segment}")
                    extended_segments_incident.append(segment) # Or skip adding it

            if extended_segments_incident:
                lc_incident = LineCollection(np.array(extended_segments_incident), colors="white", alpha=0.6, linewidths=0.8, zorder=1)
                ax.add_collection(lc_incident)

        # --- Internal Segments (not extended) ---
        ...
        if all_segments_exit:
            extended_segments_exit = []
            for segment in all_segments_exit:
                p_exit_start, p_exit_direction = segment # p_exit_start is on object, p_exit_direction defines ray path

                # CRITICAL: For this to work, p_exit_start and p_exit_direction MUST be different.
                # If they are the same (as in your original [[P2_x, P2_y], [P2_x, P2_y]]),
                # dx and dy in extend_to_edge will be 0, and extension won't happen correctly.
                # Ensure p_exit_direction is a point distinct from p_exit_start, along the desired exit path.
                
                if list(p_exit_start) == list(p_exit_direction): # More robust comparison for floats if they are tuples/lists
                    print(f"Warning: Exit segment start and direction points are identical: {segment}. Cannot extend.")
                    # Optionally, you might still want to draw a point or a short default segment
                    # For now, we'll just add the original zero-length segment if this happens,
                    # or you could choose to skip it.
                    extended_segments_exit.append(segment)
                    continue

                extended_p_end = extend_to_edge(p_exit_start, p_exit_direction, x_min, x_max, y_min, y_max)
                
                if extended_p_end:
                    extended_segments_exit.append([p_exit_start, extended_p_end])
                else:
                    # Optional: handle cases where extension is not possible
                    # print(f"Could not extend exit segment: {segment}")
                    extended_segments_exit.append(segment) # Or skip adding it

            if extended_segments_exit:
                lc_exit = LineCollection(np.array(extended_segments_exit), colors=all_colors, alpha=0.8, linewidths=1.2, zorder=1)
                ax.add_collection(lc_exit)

        # It's also good practice to set your plot limits to the canvas boundaries
        # if they aren't automatically fitting, e.g.:
...
<body>
  ...
  <script>
    ...
    function formatDisplayValue(sliderId, rawValue) {
        ...
        else if (sliderId === "ThetaI") {
            let base_angle = document.getElementById("alpha").value;
            let normal_angle = 90 + base_angle;
            let angle_of_incidence = document.getElementById(sliderId).value;
            displayValue = angle_of_incidence;
        }
      ...
      {% if task_id_for_template == "12a" and (slider.id == "ThetaI" or slider.id == "alpha" or slider.id == "canvas_scale_12a") %}
        document.getElementById("{{ slider.id }}").addEventListener("input", function(event) {
          if (document.getElementById("ThetaI") && document.getElementById("alpha")) { // Ensure both sliders exist
              ...
              var minThetaI = (alphaVal / 2.0);
              if (thetaIVal < minThetaI) {
                  thetaISlider.value = minThetaI.toFixed(stepPrecision);
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if (keyProcessed) {
            ...
            var minThetaIAfterKey = (currentAlphaVal / 2.0);
            if (currentThetaIVal < minThetaIAfterKey) {
                thetaISlider.value = minThetaIAfterKey.toFixed(stepPrecisionTheta);
            }
...
interactive_tasks = {
    ...
    "4": {
        ...
        "sliders": [
            {"id": "v_task4", "label": "Speed (m/s)", "min": round(log10(3),3), "max": round(log10(3e8),3), "value": round(log10(3e8),3), "step": 0.05, "unit": " m/s"},
    ...
    "12a": { 
        ...
        "sliders": [
            {"id": "ThetaI", "label": "Angle of Incidence at First Boundary", "min": 0, "max": 89, "value": 30, "step": 1, "unit":" deg"}, # Max 89 to avoid issues near 90 with 90-alpha/2
...
<body>
  <div class="header-banner">
    ...
    <h1>BPhO Computational Challenge 2025 - Optics</h1>
...
def generate_task11a_plot():
    ...
    ax.set_ylabel("Angle of Elevation ε (degrees from anti-solar point)")
...
def generate_task12bi_plot(): 
    ...
    frequency_THz = frequency_val / (1e12)
    ...
    title = f"Task12bi: θ_t vs θ_i given Apex α={apex_angle_deg:.0f}°, f={frequency_THz:.1f} THz, θ_max = {user_theta_max_deg_val}°"
    if ThetaMax_deg > 0:
...
def generate_task12bii_plot():
    frequency_val = 542.5e12  # Hz
    wavelength_val = 3e8 / frequency_val  # m
    alpha_val = np.pi / 4  # Apex angle in radians
    ThetaI_rad = np.linspace(0, np.pi / 2, 500)  # Incidence angles in radians
    
    # Get refractive index using the provided Sellmeier function (assumed to be available)
    n = get_refractive_index_sellmeier(np.array([wavelength_val]))[0]

    # Calculations for ray tracing through the prism
    # sin_e_arg is part of the calculation for the sine of the emergence angle (e)
    # n^2 - sin(ThetaI_rad)^2 can be negative if n*sin(alpha) < sin(ThetaI_rad)*cos(alpha) + sin(e_max_internal_reflection_limit)
    # which can happen if ThetaI_rad is large enough to cause TIR at the first surface, or if n is small.
    # However, the term under sqrt is related to refraction at the first face: n_air*sin(ThetaI) = n*sin(ThetaT1)
    # And then Snell's law at the second face.
    # The argument of arcsin (sin_e_arg) must be between -1 and 1.
    # np.maximum(0, ...) handles cases of Total Internal Reflection (TIR) inside the prism if n^2 < sin(ThetaI_rad)^2,
    # though this specific term is usually for calculating the refracted angle at the *first* surface if ThetaI was internal.
    # For external incidence, sin(ThetaT1) = sin(ThetaI_rad)/n. This is always valid if n > 1.
    # The term n^2 - sin(ThetaI_rad)^2 comes from n^2 * cos(ThetaT1)^2 where ThetaT1 is refracted angle at first face.
    
    # Angle of refraction at the first surface: sin(theta_t1) = sin(ThetaI_rad) / n
    # theta_t1 = arcsin(sin(ThetaI_rad) / n)
    # Angle of incidence at the second surface: theta_i2 = alpha_val - theta_t1
    # Angle of emergence at the second surface (e_rad): sin(e_rad) = n * sin(theta_i2)
    # sin(e_rad) = n * sin(alpha_val - arcsin(sin(ThetaI_rad)/n))
    # Using trigonometric identities: sin(A-B) = sinAcosB - cosAsinB
    # sin(e_rad) = n * (sin(alpha_val)cos(arcsin(sin(ThetaI_rad)/n)) - cos(alpha_val)sin(arcsin(sin(ThetaI_rad)/n)))
    # cos(arcsin(x)) = sqrt(1-x^2)
    # sin(e_rad) = n * (sin(alpha_val)sqrt(1 - (sin(ThetaI_rad)/n)^2) - cos(alpha_val)(sin(ThetaI_rad)/n))
    # sin(e_rad) = sin(alpha_val)sqrt(n^2 - sin(ThetaI_rad)^2) - cos(alpha_val)sin(ThetaI_rad)
    # This matches the structure of sin_e_arg.
    
    sin_e_arg = np.sin(alpha_val) * np.sqrt(np.maximum(0, n**2 - np.sin(ThetaI_rad)**2)) - np.cos(alpha_val) * np.sin(ThetaI_rad)
    
    # Calculate angle of emergence e_rad, clipping argument to [-1, 1] for arcsin domain
    e_rad = np.arcsin(np.clip(sin_e_arg, -1, 1))
    
    # Angle of deviation delta_rad
    delta_rad = ThetaI_rad + e_rad - alpha_val

    # Mask for valid emergence (where sin_e_arg was between -1 and 1, and e_rad is not NaN)
    # np.clip already handles the domain for arcsin, so e_rad should not be NaN from that.
    # The valid_mask primarily ensures that we only consider angles where emergence is physically possible.
    valid_mask = (np.abs(sin_e_arg) <= 1.0) # Redundant due to np.clip, but good for clarity
    ...
    # Plot deviation vs. incidence angle
    if len(ThetaI_deg_valid) > 0:
    ...
    # Updated title
    frequency_THz = frequency_val / 1e12
    apex_angle_deg = np.degrees(alpha_val)
    user_theta_max_deg_val = 5.787  # User-specified value for the vertical line and title
    title = f"Task12bii: Deflection angle given Apex α={apex_angle_deg:.0f}°, f={frequency_THz:.1f} THz, θ_max = {user_theta_max_deg_val}°"
    ax.set_title(title)

    # Add vertical line at the specified "minimum angle of incidence" / "theta_max"
    ax.axvline(x=user_theta_max_deg_val, color='r', linestyle=':', alpha=0.7, label=f"θ_max = {user_theta_max_deg_val}°")

    # Add legend (will pick up labels from plot and axvline)
    ax.legend(fontsize='small')
...
def generate_task12biii_plot(): 
    ...
    fig = Figure(figsize=(10, 7))
    ax = fig.add_subplot(111)
    colors_12biii = [(180, 180, 222), (171, 171, 241), (177, 187, 255), (183, 211, 255), (178, 229, 255), (183, 252, 255), (176, 255, 228), (217, 255, 213), 
                     (239, 255, 190), (255, 246, 171), (255, 213, 146), (255, 197, 168), (255, 176, 172), (235, 168, 168), (218, 183, 183)]
    hex_codes = ['#{:02x}{:02x}{:02x}'.format(*color) for color in colors_12biii]
    colors_12biii = hex_codes
    ...
    ax.set_xlabel("Angle of Incidence on First Face (degrees)")
    ax.set_ylabel("Angle of Deflection (degrees)")
    ax.set_title(f"Task 12biii: Deflection vs. Incidence for Various Apex Angles (n ≈ {n:.3f})")
